// Cloudflare Pages Function — served at /api/collection
//
// This is the only piece of the app that knows the BGG API token. It runs on
// Cloudflare's servers, never in the visitor's browser, so the token is never
// exposed in page source. Set it in the Pages project's dashboard under
// Settings > Environment variables, as a secret named BGG_TOKEN.
//
// Flow:
...
export async function onRequestGet(context) {
  ...
  if (!token) {
    return jsonResponse(
      { error: 'Server is missing its BGG_TOKEN secret. Set it in the Pages project settings.' },
      500
    );
  }
...
function jsonResponse(data, status = 200) {
  return new Response(JSON.stringify(data), {
    ...
    headers: {
      ...
      // Same-origin page calling its own Pages Function — no CORS needed,
      // but harmless to allow in case this endpoint is ever called from
...
// Cloudflare's Workers/Pages runtime has no DOMParser, and BGG's XML schema
// has been stable for well over a decade, so plain regexes over known,
// simple attribute patterns are more practical here than hand-rolling (or
// trying to bundle) a full XML parser with no build step. Every extractor
// fails soft (returns null/empty) rather than throwing, so one unexpected
// tag never takes down the whole batch.
